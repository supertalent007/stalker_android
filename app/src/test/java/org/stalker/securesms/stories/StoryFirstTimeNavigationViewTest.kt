...
class StoryFirstTimeNavigationViewTest {
  ...
  @Before
  fun setUp() {
    testSubject = StoryFirstTimeNavigationView(ContextThemeWrapper(ApplicationProvider.getApplicationContext(), org.stalker.securesms.R.style.Signal_DayNight))
  ...
  fun `Given sdk 31, when I create testSubject, then I expect overlay visible and blur hash not visible`() {
    ...
    assertTrue(testSubject.findViewById<View>(org.stalker.securesms.R.id.edu_overlay).visible)
    assertFalse(testSubject.findViewById<View>(org.stalker.securesms.R.id.edu_blur_hash).visible)
  }
  ...
  fun `Given sdk 30, when I create testSubject, then I expect overlay visible and blur hash visible`() {
    ...
    assertTrue(testSubject.findViewById<View>(org.stalker.securesms.R.id.edu_overlay).visible)
    assertTrue(testSubject.findViewById<View>(org.stalker.securesms.R.id.edu_blur_hash).visible)
  }
  ...
  fun `Given sdk 30, when I set blur hash, then blur hash is loaded`() {
    ...
    val blurHashView = testSubject.findViewById<ImageView>(org.stalker.securesms.R.id.edu_blur_hash)
